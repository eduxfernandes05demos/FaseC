...
|------|------|------|-------------|
| `legacy-src/desktop-engine/common.c` | 1286 | `sprintf(name, "%s/%s", com_gamedir, filename);` | Path construction → stack overflow |
| `legacy-src/desktop-engine/common.c` | 1424 | `sprintf(netpath, "%s/%s", search->filename, filename);` | Path construction → stack overflow |
| `legacy-src/desktop-engine/console.c` | 360 | `vsprintf(data, fmt, argptr);` | Variadic format → stack overflow |
| `legacy-src/desktop-engine/sys_win.c` | 367 | `vsprintf(text, error, argptr);` | Error message → stack overflow |
| `legacy-src/desktop-engine/sys_win.c` | 376 | `sprintf(text2, "ERROR: %s\n", text);` | Cascading overflow |
| `legacy-src/desktop-engine/gl_model.c` | 1217 | `sprintf(name, "*%i", i+1);` | Model name → limited risk |
| `legacy-src/desktop-engine/net_dgrm.c` | 134 | Format IP addresses into fixed buffers | Network data → stack overflow |
| `legacy-src/desktop-engine/vid_ext.c` | 341-362 | Mode name formatting into fixed arrays | Display mode → stack overflow |
| `legacy-src/QW/qwfwd/misc.c` | 33 | `sprintf(s, "%i.%i.%i.%i:%i", ...)` | IP formatting → overflow |
| `legacy-src/QW/qwfwd/misc.c` | 401 | `sprintf(new, "\\%s\\%s", key, value);` | User info → overflow |

**Highest-risk files** (most sprintf instances with network-adjacent data):
- `legacy-src/desktop-engine/common.c` — 7+ instances
- `legacy-src/desktop-engine/net_dgrm.c` — 3+ instances
- `legacy-src/QW/server/sv_main.c` — 5+ instances
- `legacy-src/QW/client/cl_main.c` — 5+ instances

### 2.2 strcpy / Q_strcpy (232 instances)

**Severity**: Critical — Remote Code Execution (RCE)

Unbounded `strcpy` copies source strings to destination buffers without length validation. The engine defines its own `Q_strcpy` at `legacy-src/desktop-engine/common.c:180` which is equally unsafe.

**High-Risk Examples**:

| File | Line | Code | Risk |
|------|------|------|------|
| `legacy-src/desktop-engine/common.c` | 1696 | `strcpy(com_gamedir, dir);` | Directory path from user config |
| `legacy-src/desktop-engine/common.c` | 1744 | `strcpy(basedir, com_argv[i+1]);` | Command-line arg → overflow |
| `legacy-src/desktop-engine/common.c` | 1767 | `strcpy(com_cachedir, com_argv[i+1]);` | Command-line arg → overflow |
| `legacy-src/desktop-engine/net_dgrm.c` | 132 | `Q_strcpy(addrStr, inet_ntoa(...));` | Network address → overflow |
| `legacy-src/desktop-engine/net_dgrm.c` | 558-683 | Multiple Q_strcpy with network data | Network → overflow |
| `legacy-src/desktop-engine/gl_model.c` | 203 | `strcpy(mod->name, name);` | Model name → overflow |
| `legacy-src/QW/client/cl_demo.c` | various | `strcpy(name, Cmd_Argv(1));` | User input → overflow |
| `legacy-src/QW/client/net_wins.c` | various | `strcpy(copy, s);` | Network data → overflow |
...
| File | Line | Code | Risk |
|------|------|------|------|
| `legacy-src/desktop-engine/host_cmd.c` | 275-278 | Map string concatenation | User command → overflow |
| `legacy-src/desktop-engine/host_cmd.c` | 1054-1055 | Say command text building | Chat message → overflow |
| `legacy-src/desktop-engine/pr_cmds.c` | 41 | `strcat(out, G_STRING(...));` | QuakeC string → overflow |
| `legacy-src/QW/server/sv_send.c` | 120 | `strcat(outputbuf, msg);` | Server output → overflow |
| `legacy-src/QW/server/sv_main.c` | 428 | `strcat(data, svs.log_buf[...]);` | Log data → overflow |
| `legacy-src/QW/client/cl_main.c` | 328-336 | RCON command building | Auth data → overflow |
| `legacy-src/QW/client/keys.c` | 330 | `strcat(key_lines[edit_line], textCopied);` | Clipboard paste → overflow |
...
|------|------|-----------|------|
| `legacy-src/desktop-engine/host.c` | 791 | `malloc(com_argc * sizeof(char *))` | Startup crash on OOM |
| `legacy-src/desktop-engine/host.c` | 796 | `malloc(len)` | Startup crash on OOM |
| `legacy-src/desktop-engine/gl_warp.c` | 410 | `malloc(count * 4)` | Texture crash on OOM |
| `legacy-src/desktop-engine/gl_warp.c` | 520 | `malloc(numPixels*4)` | TGA loading crash on OOM |
| `legacy-src/desktop-engine/gl_screen.c` | 618 | `malloc(glwidth*glheight*3 + 18)` | Screenshot crash on OOM |
| `legacy-src/QW/client/cl_parse.c` | 490 | `malloc(size)` | Download crash on OOM |
| `legacy-src/QW/qwfwd/qwfwd.c` | 225 | `malloc(sizeof *p)` | Forwarding crash on OOM |

### 3.2 Custom Memory Allocator Risks

The Zone allocator (`legacy-src/desktop-engine/zone.c`) implements its own heap management:
...
|------|------|------|---------|
| `legacy-src/desktop-engine/sys_linux.c` | 276 | `system(cmd);` | Executing shell commands |
| `legacy-src/QW/client/sys_linux.c` | 278 | `system(cmd);` | Executing shell commands |
...
- **RCON (remote console)**: Password sent in plaintext (`legacy-src/QW/client/cl_main.c:330`)
- **No TLS/encryption**: All network traffic is unencrypted

### 5.2 No Input Validation on Network Data

- Packet parsing trusts data from network without validation (`legacy-src/desktop-engine/net_dgrm.c:161`)
- Entity delta parsing trusts field bitmasks (`legacy-src/QW/client/cl_ents.c:265`)
...
| File | Line | Risk |
|------|------|------|
| `legacy-src/desktop-engine/common.c` | 1286 | `sprintf(name, "%s/%s", com_gamedir, filename)` — No path sanitization |
| `legacy-src/desktop-engine/common.c` | 1424 | `sprintf(netpath, "%s/%s", search->filename, filename)` — Path traversal |
| `legacy-src/QW/client/cl_parse.c` | 189 | Download temp file naming — Path traversal via server |
